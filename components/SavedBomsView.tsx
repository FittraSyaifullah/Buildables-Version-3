...
export const SavedBomsView: React.FC<SavedBomsViewProps> = ({ boms, onOpenBom, onDeleteBom }) => {
  return (
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-12">
            <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-brand-blue">Archive</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-serif font-light text-brand-darkBlue">
                Saved <span className="italic text-brand-blue">BOMs</span>
            </h1>
        ...
        {boms.length === 0 ? (
            <div className="text-center py-32 bg-white rounded-3xl border border-brand-blue/5 shadow-sm">
                <div className="w-20 h-20 bg-brand-lightBlue/30 rounded-3xl flex items-center justify-center mx-auto mb-6">
                    <Database size={32} className="text-brand-blue/30" />
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {boms.map((bom) => (
                    <div 
                        key={bom.id} 
                        className="bg-white p-8 rounded-3xl border border-brand-blue/5 shadow-[0_4px_20px_rgba(0,0,0,0.02)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.06)] hover:border-brand-blue/20 transition-all group flex flex-col h-full relative overflow-hidden"
                    >
                        <div className="flex justify-between items-start mb-8">
                            ...
                            <div className="flex gap-2">
                                <button 
                                    ...
                                    className="p-2 text-gray-300 hover:text-brand-blue hover:bg-brand-lightBlue/50 rounded-xl transition-all"
                                    title="Export CSV"
                                >
                                    <Download size={18} />
                                </button>
                        ...
                        <h3 className="font-serif font-medium text-brand-darkBlue text-2xl mb-2 group-hover:text-brand-blue transition-colors">
                            {bom.title}
                        </h3>
                        ...
                        <div className="mt-auto pt-6 border-t border-brand-blue/5 flex justify-between items-end">
                            <div>
                            ...
                            <button 
                                ...
                                className="w-12 h-12 rounded-2xl flex items-center justify-center bg-brand-lightBlue/50 group-hover:bg-brand-blue group-hover:text-white transition-all shadow-sm group-hover:translate-x-1"
                            >
                                <ArrowRight size={20} />
                            </button>
                        </div>
