      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="p-6 border-b border-gray-200 flex justify-between items-center bg-gray-50 flex-shrink-0">
            <div className="flex items-center gap-6">
                ...
                <label className="flex items-center gap-2 cursor-pointer select-none bg-white px-3 py-1.5 rounded-full border border-gray-200 shadow-sm hover:border-brand-darkBlue transition-colors">
                    <div className={`w-9 h-5 rounded-full p-0.5 transition-colors ${onlyDifferences ? 'bg-brand-darkBlue' : 'bg-gray-300'}`}>
                    ...
                    <tr>
                        ...
                        {parts.map((part, idx) => (
                            <th key={part.id} className={`p-4 min-w-[240px] border-b border-r border-gray-200 bg-white sticky top-0 z-10 text-left relative ${part.isReference ? 'bg-blue-50/30' : ''}`}>
                                {part.isReference && (
                                    <div className="absolute top-0 left-0 right-0 h-1 bg-brand-darkBlue"></div>
                                )}
                                <div className="mb-2">
                                ...
                                <div className="flex gap-2 mt-3">
                                    <button className="flex-1 py-1.5 bg-brand-orange text-white rounded text-xs font-bold shadow hover:bg-orange-600 transition-colors">
                                        Select
                                    </button>
                                    <button className="p-1.5 border border-gray-200 rounded text-gray-500 hover:text-brand-darkBlue hover:border-brand-darkBlue transition-colors" title="Datasheet">
                                        <FileText size={14} />
                                    </button>
                                </div>
                            </th>
